...
public class DeviceActivity extends MyActivity implements MooshimeterDelegate {
    ...
    //private CooldownTimer broadcast_cooldown = new CooldownTimer();
    ...
    protected void onPause() {
        ...
        if(! (  mMeter.speech_on.get(MooshimeterControlInterface.Channel.CH1) ||
                mMeter.speech_on.get(MooshimeterControlInterface.Channel.CH2) ||
                Util.getPreferenceBoolean(Util.preference_keys.BROADCAST_INTENTS) ) ) {  // 8Feb2017 added so broadcast intents run in background, the same as speaking
